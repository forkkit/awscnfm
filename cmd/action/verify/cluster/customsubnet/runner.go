...
func (r *runner) run(ctx context.Context, cmd *cobra.Command, args []string) error {
...

	{
		...
		for _, cluster := range list.Items {
			...
			customMask := net.IPv4Mask(0xff, 0xff, 0xfe, 0x00)

			_, cpMask, err := net.ParseCIDR(subnetString)
			if err != nil {
				return microerror.Mask(err)
			}

			// This only checks if the IP part of the CIDR is inside the NetworkPool CIDR
			if !reflect.DeepEqual(cpMask.Mask, customMask) {
				return microerror.Mask(subnetMaskMasterUnusedError)
			}
		}
	}

	{
		...
		for _, md := range list.Items {
			...
			customMask := net.IPv4Mask(0xff, 0xff, 0xff, 0xf0)

			_, mdNet, err := net.ParseCIDR(subnetString)
			if err != nil {
				return microerror.Mask(err)
			}

			// This only checks if the IP part of the CIDR is inside the NetworkPool CIDR
			if !reflect.DeepEqual(mdNet.Mask, customMask) {
				return microerror.Mask(subnetMaskWorkerUnusedError)
			}
		}
	}
